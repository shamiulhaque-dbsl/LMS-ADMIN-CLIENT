...
import SortSectionModal from "../modals/section-sort/SortSectionModal";
import { SectionModal } from "../modals/SectionModal";
import { Tooltip } from "@/components/ui/Tooltip";

export const CurriculumForm = () => {
  const { sections, deleteSection } = useCurriculumStore();
  const openModal = useModalStore((s) => s.openModal);
  const closeModal = useModalStore((s) => s.closeModal);

  console.log("CurriculumForm sections:", sections);
  ...
  return (
    <div className="py-4">
      ...
      <div className="flex justify-center gap-4">
        ...
        <Button type="button" size="sm" onClick={() => openModal("lesson-modal")}>
          <Icons.plus size={16} />
          <Text as="span" className="ml-2">
            Add Lesson
          </Text>
        ...
        {sections.map((section, index) => (
          <div key={section.id} className="rounded-md bg-blue-200/20 p-4">
            ...
            <div className="flex justify-between items-center mb-6">
              <Text>
                Section {index + 1}: <b>{section.title}</b>
              </Text>

              <div className="flex gap-3">
                <Button size="sm" onClick={() => openModal("section-modal")}>
                  <Icons.edit size={16} />
                ...
                <Button size="sm" onClick={() => openModal("delete-modal")}>
                  <Icons.trash size={16} />
                  <Text as="span" className="ml-1">
                    Delete
                  </Text>
            ...
            <Grid>
              {section.lessons.map((lesson, idx) => (
                <Card key={lesson.id} className="bg-white border-none">
                  <Card.Content className="flex justify-between p-3">
                    <Text>
                  ...
                  <div className="flex items-center justify-end">
                    <Tooltip content="Edit Lesson">
                      <Button type="button" size="sm" className="px-2 text-indigo-400">
                        <Icons.fileText size={16} />
                      </Button>
                    </Tooltip>
                    <Tooltip content="Resources Files">
                      <Button type="button" size="sm" className="px-2 text-indigo-700">
                        <Icons.edit size={16} />
                      </Button>
                    </Tooltip>
                    <Tooltip content="Delete Lesson">
                      <Button type="button" size="sm" className="px-2 text-red-400">
                        <Icons.trash size={16} />
                      </Button>
                    </Tooltip>
                  </div>
      ...
      <SectionModal />
      <LessonModal onSave={() => {}} sectionOptions={sections} />
      <SortSectionModal />
      <ConfirmationModal
        id="delete-modal"
        type="delete"
